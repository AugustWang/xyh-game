...
loop(State) ->
	...
	{tcp_error, Sock, Reason} ->
		...
		%% add by wenziyong, sleep 1000 ms in order to ensure parent can recv the msg
		%% even if sleep some time, can't ensure parent recv the msg
		%timer:sleep(1000),
	    error;
	{tcp_closed, Sock} ->
	    ...
	    LogFun(?MODULE, ?LINE, error,
		   fun() ->
			   {"--------mysql_recv: Socket ~p closed,recv pid ~p exit", [Sock,self()]}
		   end),
		...
		%timer:sleep(1000),
	    error;
	%% add by wenziyong,if recv unknow msg also return error
	Unkown->
	    LogFun = State#state.log_fun,
	    LogFun(?MODULE, ?LINE, error,
		   fun() ->
			   {"--------mysql_recv unkown msg:~p, Socket ~p ,recv pid ~p exit", [Unkown,Sock,self()]}
		   end),
	    State#state.parent ! {mysql_recv, self(), closed, {error, "unkown msg"}},
	    error
