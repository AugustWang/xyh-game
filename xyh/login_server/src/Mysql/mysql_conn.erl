...
%% add by wenziyong, in order to maintain the connect, send a query when the connect is free for an hour 
-define(CONNECT_MAX_FREE_TIME, 3600000).
...
post_start(Pid, LogFun) ->
	...
	Unknown ->
	    ?Log2(LogFun, error,
		 ...
		 post_start(Pid, LogFun)
    after 10000 ->  %% modified by wenziyong from 5000
	    {error, "timed out"}
    end.
	...
	{ok, RecvPid, Sock} ->
		...
		{ok, Version} ->
			...
			{_ResultType, _MySQLRes} ->
				...
				%% add by wenziyong
			    ?Log2(LogFun, debug,
				"mysql_recv info to ~p (receive pid ~p)", [PoolId, RecvPid]),
				%% RecvPid send msg: {mysql_recv, RecvPid, closed, _E} to notify the socket is closed, when recv it in loop function, exit the process
				%% But do_recv alse handle the msg:msg: {mysql_recv, RecvPid, closed, _E}, then loop function maybe never recv it
				%% so monitor recvPid, let loop function recv 'DOWN' msg, then exit
				erlang:monitor(process, RecvPid),
				
				%% add by wenziyong
				%% mysql:start_reconnect will lose the link with mysql_dispatcher, link to mysql_dispatcher
				%% it's hardcode, if not need link,please comment the following lines
				link(whereis(mysql_dispatcher)),
...
loop(State) ->
	...
	{mysql_recv, RecvPid, data, Packet, Num} ->
	    ...
	    loop(State);
	%% add by wenziyong, handle DOWN message when recvPid exit
	{'DOWN', _MonitorRef, process, _Pid, Info} ->
		%% RecvPid exit
		?Log2(LogFun, error,
		  "-----connect exit, received DOWN message, exiting: ~p,connect pid:~p, recv pid:~p", [Info,self(),State#state.recv_pid]),
	    error;	
	Unknown ->
	    ?Log2(LogFun, error,
		  "-----connect exit, received unknown signal, exiting: ~p,connect pid:~p, recv pid:~p", [Unknown,self(),State#state.recv_pid]),
		%% if normal exit,mysql_recv process don't exit
		%% so let's exit with exception, then mysql_recv will exit
		throw(Unknown)
	%% add by wenziyong, in order to maintain the connect, send a query when the connect is free for a long time 
	after ?CONNECT_MAX_FREE_TIME ->
		io:format("----send query to maintain the connect ~n"),
		do_query(State,<<"SELECT 1">>),
	    loop(State)
    end.
...
do_query(Sock, RecvPid, LogFun, Query, Version) ->
	...
	{error, Reason} ->
	    Msg = io_lib:format("Failed sending data "
				"on socket : ~p",
				[Reason]),
		%% add by wenziyong for debug
		?Log2(LogFun, error,"-------------------------------------------------------------do_send fail in do_query ~s ~n", [Msg]),
