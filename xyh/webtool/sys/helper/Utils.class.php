...
class Utils {
    ...
    public static function pager($url, $totalRecord, $currentPage, $limit, $half = 5) {
        ...
        $html = ($currentPage > 0) ? " <a href='$url&page=" . ($currentPage - 1) . "' title='上一页'>&laquo;</a>" : " <span class='d'>&laquo;</span>";
        ...
        for(; $i < $j; $i ++) {
            $html .= ($i == $currentPage) ? " <span class='c'>" . ($i + 1) . "</span>" : " <a href='$url&page=$i'>" . ($i + 1) . "</a>";
        }
        $html .= ($currentPage + 1 < $totalPage) ? " <a href='$url&page=" . ($currentPage + 1) . "' title='下一页'>&raquo;</a>" : " <span class='d'>&raquo;</span>";
        $jump = ($half * 2 + 1) < $totalPage ? " <a href='javascript:void(0)' onclick=\"var page=prompt('请输入你想跳转到的页数(1-{$totalPage}):','');if(page>0 && page<=$totalPage){window.location.replace('$url'+'&page='+(page-1));}\" title='快速跳转页面'>&raquo;&raquo;</a>" : '';
        return $html . $jump . ' <span class="t">' . ($currentPage + 1) . "/$totalPage 共{$totalRecord}条记录</span>";
    }
    /**
     * 查IP所在地(需纯真IP数据包)
     * 来自discuz
