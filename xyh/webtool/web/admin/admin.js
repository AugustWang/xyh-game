...
function customRange(input) {  
    return {
        minDate: (input.id == 'kwDateEnd' ? $('#kwDateBegin').datepick('getDate') : null),  
        maxDate: (input.id == 'kwDateBegin' ? $('#kwDateEnd').datepick('getDate') : null)};  
}
function Q(url, item){
...
$(document).ready(function(){
    ...
    $('.js_starttime').datepick({dateFormat:'yyyy-mm-dd 00:00:00'});
    $('.js_endtime').datepick({dateFormat:'yyyy-mm-dd 23:59:59'});
    
    //范围日期选择必须使用'kwDateBegin'和'kwDateEnd'这两个ID
    if($('#kwDateBegin').size() && $('#kwDateEnd').size()){
        $('#kwDateBegin, #kwDateEnd').datepick({
