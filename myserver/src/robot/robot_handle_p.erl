...
handle(11003, [Diamond, Gold, Gid, Power
    ...
    case Gid < 50 of
        true ->
            timer:apply_after(100,
                lib_admin, set_tollgate, [R#robot.id, 50]);
        false ->
            ok
    end,
    case Name of
    ...
    case BagEqu of
        EN when EN < 160 ->
            ELine = EN div 8 + 1,
            erlang:send_after(util:rand(1000, 3000),
                R#robot.pid_sender, {cmd, 13027, [1, ELine, 5]}),
            ok;
        _ ->
            ok
    end,
    case BagProp of
        PN when PN < 160 ->
            PLine = PN div 8 + 1,
            erlang:send_after(util:rand(1000, 3000),
                R#robot.pid_sender, {cmd, 13027, [1, PLine, 2]}),
            ok;
        _ ->
            ok
    end,
    case BagMat of
        MN when MN < 160 ->
            MLine = MN div 8 + 1,
            erlang:send_after(util:rand(1000, 3000),
                R#robot.pid_sender, {cmd, 13027, [1, MLine, 1]}),
            ok;
        _ ->
            ok
    end,
    ...
    timer:apply_after(3000,
        lib_admin, add_item, [R#robot.id, 15001, 4]),
    timer:apply_after(3000,
        lib_admin, add_item, [R#robot.id, 15002, 4]),
    R#robot{
        gold            = Gold
        ,diamond        = Diamond
        ,bag_equ_max    = BagEqu
...
handle(22002, [Power|_], R) ->
    ...
    lists:foreach(fun(Id) ->
        ...
        end, lists:seq(20001,20006)),
    erlang:send_after(Time, R#robot.pid_sender, {cmd, 22002, [1, 1, 2]}),
...
handle(25002, _Data, R) ->
    ...
    R#robot.pid_sender ! {cmd, 13003, [[[1, 1, 9]]]},
    %% 请求玩家所有英雄
    R#robot.pid_sender ! {cmd, 14002, []},
    ok;
