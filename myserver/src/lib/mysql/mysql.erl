...
handle_info({'DOWN', _MonitorRef, process, Pid, Info}, State) ->
    %% F = "~nreceived 'DOWN': ~w~n",
    %% A = [{'DOWN', _MonitorRef, process, Pid, Info}],
    %% case is_pid(whereis(cc_logger)) of
    %%     true -> cc_logger:notify(warning, F, A, ?MODULE, ?LINE);
    %%     false -> io:format(F, A)
    %% end,
    LogFun = State#state.log_fun,
...
handle_info({del_pool, PoolId}, State) ->
    ConnPools = State#state.conn_pools,
    {ConnPools1, Pids} = case gb_trees:lookup(PoolId, ConnPools) of
        none -> {ConnPools, []};
        {value, {Unused, Used}} ->
            ConnList = Unused ++ Used,
            Pids1 = [erlang:element(3, X) || X <- ConnList],
            {gb_trees:delete_any(PoolId, ConnPools), Pids1}
    end,
    PidsPools = lists:foldl(
        fun(Pid, Acc) ->
                Pid ! close_conn,
                gb_trees:delete_any(Pid, Acc)
        end
        ,State#state.pids_pools
        ,Pids
    ),
    State1 = State#state{conn_pools = ConnPools1, pids_pools = PidsPools},
    {noreply, State1};

handle_info(_Msg, State) ->
    io:format("~nundefined msg:~p", [_Msg]),
    {noreply, State}.
