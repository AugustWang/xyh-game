%%----------------------------------------------------
%% $Id: mod_item.erl 13274 2014-06-21 04:48:00Z rolong $
%%
%% @doc 物品模块
...
jewelry_broken(Rs) ->
    #role{items = Items} = Rs,
    {Rt1,Rt2} = jewelry_broken(Items, [], []),
    Rs1 = case Rt1 == [] of
        true -> Rs#role{items = Rt2};
        false ->
            Rs#role{items = Rt2, save = [items]}
    end,
    Tids = jewelry_add(Rt1, []),
    case mod_item:add_items(Rs1, Tids) of
        {ok, Rs2, PA, EA} ->
            mod_item:send_notice(Rs#role.pid_sender,PA,EA),
            {ok, Rs2#role{save = [items]}};
        {error, full} ->
            Title = data_text:get(2),
            Body  = data_text:get(4),
