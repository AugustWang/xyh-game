%% ------------------------------------------------------------------
%% $Id: mod_colosseum.erl 13274 2014-06-21 04:48:00Z rolong $
%% @doc 新版竞技场
...
handle_cast({set_coliseum_rank,Pid,Id,Rid,Name,Pic,Power}, State) ->
    Ids = data_coliseum:get(ids),
    Data = data_coliseum:get(length(Ids)),
...
handle_info(update_coliseum_rank, State) ->
    ...
    List2 = fix_coliseum_rank2(List1),
    put(coliseum_rank, List2),
...
check_init_rank_from_db() ->
    Sql = list_to_binary([<<"SELECT count(*) FROM `arena` WHERE `index` <> 0">>]),
...
%% 重启服务排行榜初始化
init_coliseum_rank() ->
    Sql = list_to_binary([<<"SELECT `index`, `id`, `rid`, `name`, `picture`, `power` FROM `arena` WHERE `index` <> 0 ORDER BY `index` ASC">>]),
    case db:get_all(Sql) of
        {ok, Data} -> Data;
        {error, null} -> [];
        {error, Reason} ->
