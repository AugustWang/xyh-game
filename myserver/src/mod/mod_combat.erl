%%----------------------------------------------------
%% $Id: mod_combat.erl 13256 2014-06-18 05:40:49Z rolong $
%%
%% @doc 战斗模块
        ...
        {3, Type} ->
            ...
            case mod_colosseum:get_data(ArenaId) of
                {Robot, Rid, _Lev, _, Name, S} ->
                    ...
                    {Over, InitHp, Data2, _, _, _} = mod_battle:battle(arena, Heroes1, Heroes2),
                    ...
                    {Rs0, [Data3, Data4]} = case IsWin == 1 of
                        true ->
                            mod_item:drop_produce_arena(Rs4, Lev1);
                        false ->
                            {Rs4, [[], []]}
                    end,
                    {[Rs0#role.power, 0, IsWin, 0, InitHp, Data2, [], Data3, Data4], Rs0};
                {error, Reason} ->
        ...
        true ->
            ...
            case Power1 >= 0 of
                true ->
                    ...
                    {Over, InitHp, Data2, _OverHP, OverHeroes, Bout} = mod_battle:battle(main, Heroes1, Heroes22),
                    ...
                    case Over =:= 1 of
                        true ->
                            ...
                            {Power1, 1, Rs7, HeroesInfo, Data6, Data7};
                        false ->
                            {Power, 0, Rs, [], [], []}
                    end,
                    %% 主线战斗成就推送
                    ...
                    {Rs000,Star} = case IsWin == 1 andalso lists:keymember(nightmare,1,Data) of
                        true ->
                            ...
                            InitHeroes1 = erlang:length(Heroes1),
                            ...
                            Star3 = case Bout =< 20 of
                                true -> Star2 bor 4;
                                false -> Star2
                            end,
                            Fbn = lists:keystore(ReqPassId,1,Nightmare,{ReqPassId,Star3}),
                            {Rs00#role{fb_nightware = Fbn,save = [fbinfo]},Star3};
                        false -> {Rs00,0}
                    end,
                    {[Power2, PassId, IsWin, Star, InitHp, Data2, Data3, Data4, Data5], Rs000};
                false ->
        ...
        true ->
            ...
            case Power1 >= 0 of
                true ->
                    ...
                    HeroesTmp = [XX || XX <- Rs#role.heroes, XX#hero.pos > 0],
                    mod_battle:print_hero(HeroesTmp),
                    {Over, InitHp, Data2, _, _, _} = mod_battle:battle(fb, Heroes1, Heroes2),
                    {IsWin, Rs3, Data5, Data6} =
                    case Over =:= 1 of
                        true ->
                            %% Rs1 = mod_attain:combat_attain(ReqPassId, Rs),
                            %% 请求战斗掉落
                            %% #role{produce_pass = ProducePass} = Rs,
                            {Rs2, [Data3, Data4]} = mod_item:drop_produce(Rs, {2, ReqPassId}),
                    ...
                    {[Rs5#role.power, ReqPassId, IsWin, 0, InitHp, Data2, [], Data5, Data6], Rs5#role{power = Power1}};
                false ->
                    ?WARN("Power: ~w -> ~w", [Power, Power1]),
                    {[0, 0, 0, 0, [], [], [], [], []], Rs}
            end
...
%%             HeroesTmp = [XX || XX <- Rs#role.heroes, XX#hero.pos > 0],
%%             mod_battle:print_hero(HeroesTmp),
%%             {Over, InitHp, Data2, _, _, _} = mod_battle:battle(fest, Heroes1, Heroes2),
                        ...
                        {ok, Items1, Notice} ->
                            ...
                            case Over =:= 1 of
                                true ->
                                    %% Rs1 = mod_attain:combat_attain(ReqPassId, Rs),
                                    %% 请求战斗掉落
                                    {Rs2, [Data3, Data4]} = mod_item:drop_produce(Rs, {5, ReqPassId}),
                                    {1, Rs2, Data3, Data4};
                                false -> {0, Rs, [], []}
                            end,
                            Combat = mod_hero:calc_power(Heroes1),
                            ...
                            Rs5 = Rs4#role{power = Power1, items = Items1},
                            {[Rs5#role.power, ReqPassId, IsWin, 0, InitHp, Data2, [], Data5, Data6], Rs5};
                        {error, _} ->
