%%----------------------------------------------------
%% $Id: pt_fest.erl 13256 2014-06-18 05:40:49Z rolong $
%% @doc 协议25 - 节日活动
...
handle(25002, [], Rs) ->
    ...
    List7 = [[Id,V,N,S] || {Id,V,N,S} <- List6],
    ?DEBUG("===List7:~w, ===ActIds:~w", [List7, ActIds]),
...
handle(25030, [], Rs) ->
    ...
    {?WEIXINSHARE, V, N, _St} = lists:keyfind(?WEIXINSHARE, 1, ActIds),
    ...
    case T > 0 of
        ...
        false ->
            ...
            List2 = lists:keyreplace(?WEIXINSHARE, 1, ActIds, {?WEIXINSHARE, ShareTime, N, 1}),
            %% ?INFO("=====List2:~w", [List2]),
            Rs1 = role:save_delay(fest, Rs),
            {ok, [0, WeixinCD], Rs1#role{activity = List2}}
    end;

%% 微信分享领奖
handle(25031, [], Rs) ->
    #role{activity = ActIds} = Rs,
    {?WEIXINSHARE, V, N, St} = lists:keyfind(?WEIXINSHARE, 1, ActIds),
    case St =:= 1 of
        true ->
            case data_fest_prize:get({?WEIXINSHARE, N}) of
                undefined -> {ok, [127]};
                Data ->
                    ...
                    case mod_item:add_items(Rs, Tid) of
                        {ok, Rs1, PA, EA} ->
                            ...
                            NewAct = lists:keyreplace(?WEIXINSHARE, 1, ActIds, {?WEIXINSHARE, V, N, 2}),
                            %% ?INFO("=====NewAct:~w", [NewAct]),
                            Rs3 = role:save_delay(fest, Rs2),
                            {ok, [0, Cd], Rs3#role{activity = NewAct, save = [items]}};
                        {error, full} ->
                            Title = data_text:get(2),
                            Body = data_text:get(4),
                            ...
                            NewAct = lists:keyreplace(?WEIXINSHARE, 1, ActIds, {?WEIXINSHARE, V, N, 2}),
                            Rs1 = role:save_delay(fest, Rs),
                            {ok, [3, Cd], Rs1#role{activity = NewAct}};
                        {error, _} ->
                            {ok, [129, 0]}
                    end
            end;
...
handle(25032, [], Rs) ->
    ...
    {_, _, 33, Cond, _} = lists:keyfind(33, 3, Attain),
