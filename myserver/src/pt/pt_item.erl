%%----------------------------------------------------
%% $Id: pt_item.erl 13274 2014-06-21 04:48:00Z rolong $
%% @doc 协议13 - 物品相关
                                                                ...
                                                                {ok, Status, EquItem1} ->
                                                                    ...
                                                                    Rs4 = case Status of
                                                                        0 ->
                                                                            ...
                                                                            mod_attain:attain_state(14, 1, Rs3);
                                                                        1 -> mod_attain:attain_state(16, 1, Rs3);
                                                                        _ -> Rs3
                                                                    end,
                                                                    Rs6 = case Diamond > 0 of
                                                                        true ->
                                                                            mod_attain:attain_state(29, 1, Rs4);
...
handle(13012, [List], Rs) ->
    ...
    case forge(List, Rs) of
        [0,Tidss,Rs1] ->
            ...
            lib_role:notice(Rs5),
            {ok, [0, Tidss], Rs5#role{save = [items]}};
        [3,Tidss,Rs1] ->
            Rs3 = case Rs1#role.diamond < Rs#role.diamond of
                true ->
                    lib_role:spend_ok(diamond,3,Rs,Rs1);
            ...
            {ok, [3, Tidss], Rs5#role{save = [items]}};
        [R] -> {ok, [R, []]}
    end;
...
handle(13025, [Id,Ids], Rs) ->
    ...
    AddExp = jewelry_upgrade_exp(Ids, Items),
        ...
        Item1 ->
            ...
            case is_record(Attr1,prop) andalso Sort1 == 4 of
                true ->
                    ...
                    Exp2 = Exp1 + AddExp,
                    Level2 = jewelry_upgrade(Level1,Quality1,Exp2),
                    Attr2 = Attr1#prop{
                        level = Level2
                        ,exp  = Exp2
                    },
                        ...
                        {RewardTid, Num, Quality, Pos, Val} ->
                            case mod_item:add_item(Rs, RewardTid, Num) of
                                {ok, Rs1, PA, EA} ->
                                    ...
                                    %% 幸运星排行榜设置
                                    %% gen_server:cast(rank, {rank_set, ValSum1, Rid, Name, 1}),
                                    Rs0 = mod_attain:attain_state(46, 1, Rs5),
                                    case Quality =:= 2 of
                                        true ->
                                            Vip = lib_admin:get_vip(Rs),
                                            Name2 = lib_admin:get_name(Rs),
...
%% 签到续签
handle(13054, [], Rs) ->
    #role{loginsign = LoginSign, loginsign_type = Type} = Rs,
    ...
    case Days1 == Days2 andalso Days1 == 1 andalso Days1 == 7 of
        ...
        false ->
            ...
            case lib_role:spend(diamond, Diamond, Rs) of
                {ok, Rs1} ->
                    Rs2 = Rs1#role{loginsign = [Days2, Days2, PriceState]},
                        ...
                        true ->
                            ...
                            case mod_item:add_items(Rs4, TidNum) of
                                {ok, Rs5, PA, EA} ->
                                    ...
                                    case PA =:= [] andalso EA =:= [] of
                                        true ->
                                            ...
                                            lib_role:notice(Rs0),
                                            lib_role:notice(luck, Rs0),
                                            {ok, [0, Next], Rs0#role{loginsign = [Days1, Days2, Lists2], save = [sign]}};
                                        false ->
                                ...
                                {error, full} ->
                                    ...
                                    {ok, [3, Next], Rs0#role{loginsign = [Days1, Days2, Lists2], save = [sign]}};
                                {error, _} ->
                                    {ok, [128, 1]}
                            end;
                        flase ->
                        ...
                        Item ->
                            ...
                            if
                                ...
                                true ->
                                    ...
                                    {ok, Rs#role{items = Items12, heroes = Heroes1}}
                            end
                    end;
                false ->
                    %% 卸载装备
                        ...
                        {ok, Items1, Notice} ->
                            case util:rate(Rate) of
                                true ->
                                    ...
                                    forge(Tids, Rs5, [{Tid,Notice,0}|Rt]);
                                false ->
                                    %% 失败成就推送和是否损毁
                                    Rs2 = case Del == 1 of
                                        true -> Rs1#role{items = Items1};
                                    ...
                                    Rs3 = mod_attain:attain_state(22, 1, Rs2),
                                    Rs4 = mod_attain:attain_state(43, 1, Rs3),
                                    forge(Tids,Rs4,[{Tid,Notice,1}|Rt])
                            end;
                        {error, _} ->
                            [2]
                    end;
...
jewelry_upgrade_exp(Ids, Items) ->
    jewelry_upgrade_exp(lists:flatten(Ids), Items, 0).
jewelry_upgrade_exp([], _Items, Sum) -> Sum;
jewelry_upgrade_exp([Id|Ids], Items, Sum) ->
    Sum1 = case mod_item:get_item(Id, Items) of
        false -> 0;
        Item ->
            Sort = Item#item.sort,
            Attr = Item#item.attr,
    ...
    jewelry_upgrade_exp(Ids, Items, Sum + Sum1).
