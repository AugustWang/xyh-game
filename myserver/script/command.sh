...
SVN_DIR=/home/dev/
MYSQLROOT_PASSWD=`cat /root/.mysqlroot |grep "mysqlroot_passwd"|cut -f 2 -d "="`
DATABASE_NAME=chuichui
...
export_release (){
    ...
    # $MYSQL_BIN_DIR/mysqldump -d -uroot -p"$MYSQLROOT_PASSWD" $DATABASE_NAME > $REL_DIR/$DATABASE_NAME.sql
    tar zcf cc_release.tar.gz cc_release
    echo "Created: cc_release.tar.gz"
}
##.
