...
-record(role, {
        ...
        %% ------------------------------------------------------------------
        %% Time=签到时间 loginsign_time
        %% type=奖励类型 loginsign_type
        %% day1=当前签到天数
        %% day2=已经签到天数
        %% State:0=未签到,1=可领取,2=已领取
        %% [Time, type, day1, day2, [{day, State}]]
        %% ------------------------------------------------------------------
        ,loginsign = []
        ...
        %% pos = 位置
        %% state:0=正常,1=已经被抽取
        %% [cd1,cd2,time,num,[{pos,id,state,rare}]]
        ,herosoul = []
        ,fundata = []  %% [{id,num}..]
        ,task = []   %% [{Id,target,[{C1,C2}..],State}..]
    }
).
