%%----------------------------------------------------
%% $Id: connection.erl 8065 2014-01-09 02:09:16Z rolong $
%% @doc Socket收包/发包服务
...
handle_info(shutdown, State) ->
    State#state.pid_role ! shutdown2,
...
process(sync, PidRole, PidSender, Cmd, Data) ->
    T = erlang:now(),
    case catch gen_server:call(PidRole, {pt, Cmd, Data}, 6000) of
        noreply -> ok;
        {'EXIT', Reason} ->
            A = lists:foldl( fun(P, Acc0) -> [{P, erlang:process_info(P, registered_name), erlang:process_info(P, memory), erlang:process_info(P, message_queue_len), erlang:process_info(P, current_function), erlang:process_info(P, initial_call)} | Acc0] end, [], [PidRole]),
            io:format("sender:~n~p", [A]),
            ?WARNING("Error when call Role! [CMD=~w, DATA=~w, Reason:~w]", [Cmd, Data, Reason]),
            sender:send_code(PidSender,1009),
            ok;
        Reply ->
