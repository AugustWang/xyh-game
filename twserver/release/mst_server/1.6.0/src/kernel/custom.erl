...
set(Val,Content,Items,Start_time,End_time) ->
    ...
    gen_server:cast(?MODULE, {set_custom, Val,Content,Items,Start_time,End_time}).
...
get() ->
    ets:foldl(fun get_custom_val1/2, [], ?MODULE).

get_custom_val1(H, Rt) ->
...
handle_cast({set_custom,Val,Content,Items,Start_time,End_time}, State) ->
    Key = ets:info(?MODULE, size),
    NewKey = get_new_key(Key),
    Custom = #custom{
        key = NewKey
