%% ------------------------------------------------------------------
%% $Id: lib_role.erl 13181 2014-06-04 13:36:59Z rolong $
%% @doc 角色相关API库
...
db_init1(paydouble, Rs) ->
    case mod_fest:init_pay_double(Rs) of
        {ok,Rs1} ->db_init1(fundata,Rs1);
        {error, Reason} -> {error, Reason}
    end;
db_init1(fundata, Rs) ->
    case mod_item:init_fundata(Rs) of
        {ok,Rs1} ->db_init1(task,Rs1);
        {error, Reason} -> {error, Reason}
    end;
db_init1(task, Rs) ->
    case mod_task:init_task(Rs) of
        {ok,Rs1} ->db_init1(herosoul,Rs1);
        {error, Reason} -> {error, Reason}
    end;
db_init1(herosoul, Rs) ->
    case mod_item:init_herosoul(Rs) of
        {ok,Rs1} ->db_init1(gatehero,Rs1);
        {error, Reason} -> {error, Reason}
    end;
db_init1(gatehero, Rs) ->
    mod_hero:init_gatehero(Rs).
...
%% 保存gatehero数据
save_to_db(Rs, [gatehero | S]) ->
    T = erlang:now(),
    case mod_hero:update_gatehero(Rs) of
        {ok, _} ->
            DT = timer:now_diff(erlang:now(), T) / 1000,
            ?DEBUG("Rid: ~w, save gatehero: ~w ms", [Rs#role.id, DT]),
            save_to_db(Rs, S);
        {error, _} ->
            Rs2 = role:save_delay([gatehero | S], Rs),
            {false, Rs2#role{save = []}}
    end;

%% 保存herosoul数据
save_to_db(Rs, [herosoul | S]) ->
    T = erlang:now(),
    case mod_item:update_herosoul(Rs) of
        {ok, _} ->
            DT = timer:now_diff(erlang:now(), T) / 1000,
            ?DEBUG("Rid: ~w, save herosoul: ~w ms", [Rs#role.id, DT]),
            save_to_db(Rs, S);
        {error, _} ->
            Rs2 = role:save_delay([herosoul | S], Rs),
            {false, Rs2#role{save = []}}
    end;

%% 保存fundata数据
save_to_db(Rs, [fundata | S]) ->
    T = erlang:now(),
    case mod_item:update_fundata(Rs) of
        {ok, _} ->
            DT = timer:now_diff(erlang:now(), T) / 1000,
            ?DEBUG("Rid: ~w, save fundata: ~w ms", [Rs#role.id, DT]),
            save_to_db(Rs, S);
        {error, _} ->
            Rs2 = role:save_delay([fundata | S], Rs),
            {false, Rs2#role{save = []}}
    end;

%% 保存task数据
save_to_db(Rs, [task | S]) ->
    T = erlang:now(),
    case mod_task:update_task(Rs) of
        {ok, _} ->
            DT = timer:now_diff(erlang:now(), T) / 1000,
            ?DEBUG("Rid: ~w, save task: ~w ms", [Rs#role.id, DT]),
            save_to_db(Rs, S);
        {error, _} ->
            Rs2 = role:save_delay([task | S], Rs),
            {false, Rs2#role{save = []}}
    end;
