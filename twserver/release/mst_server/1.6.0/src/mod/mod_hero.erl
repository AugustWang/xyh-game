%%----------------------------------------------------
%% $Id: mod_hero.erl 13210 2014-06-05 08:41:45Z piaohua $
%%
%% @doc Hero
...
%% 获取出战的英雄并计算属性
%% (英雄基础属性 * 品质系数 + 英雄基础属性 * 品质系数 * LV/25) + 装备属性
...
get_combat_heroes(Heroes, Items) ->
    F = fun(H) ->
            ...
            H1 = H#hero{
                ...
                %% hp         = util:floor((Hp       * Lev/25 + Hp      ) * Arg2 )
                %% ,atk       = util:floor((Atk      * Lev/25 + Atk     ) * Arg2 )
                %% ,def       = util:floor((Def      * Lev/25 + Def     ) * Arg2 )
                %% ,pun       = util:floor((Pun      * Lev/25 + Pun     ) * Arg2 )
                %% ,hit       = util:floor((Hit      * Lev/25 + Hit     ) * Arg2 )
                %% ,dod       = util:floor((Dod      * Lev/25 + Dod     ) * Arg2 )
                %% ,crit      = util:floor((Crit     * Lev/25 + Crit    ) * Arg2 )
                %% ,crit_num  = util:floor((CritNum  * Lev/25 + CritNum ) * Arg2 )
                %% ,crit_anti = util:floor((CritAnit * Lev/25 + CritAnit) * Arg2 )
                %% ,tou       = util:floor((Tou      * Lev/25 + Tou     ) * Arg2 )
                hp         = util:floor(Hp       * Lev/25 + Hp      )
                ,atk       = util:floor(Atk      * Lev/25 + Atk     )
                ,def       = util:floor(Def      * Lev/25 + Def     )
                ,pun       = util:floor(Pun      * Lev/25 + Pun     )
                ,hit       = util:floor(Hit      * Lev/25 + Hit     )
                ,dod       = util:floor(Dod      * Lev/25 + Dod     )
                ,crit      = util:floor(Crit     * Lev/25 + Crit    )
                ,crit_num  = util:floor(CritNum  * Lev/25 + CritNum )
                ,crit_anti = util:floor(CritAnit * Lev/25 + CritAnit)
                ,tou       = util:floor(Tou      * Lev/25 + Tou     )
            },
            %% ?DEBUG("Hero Lev Add:", []),
...
attr_val(Attr, Data, Arg) ->
    ...
    Offset = data_fun:hero_offset(),
    VV = util:ceil(V * Offset * Arg),
    %% ?INFO("~w: V(~w) * O(~w) * Q(~w) = VV(~w)", [Attr, V, Offset, Arg, VV]),
    VV.
...
upgrade_star(Hero) ->
    ...
    Data = data_hero_star:get(Hid),
    Arg = case util:get_val(star,Data) of
        undefined -> 1;
        S when Star < size(S) -> element(Star+1,S) / 100;
        _ -> 1
    end,
    NewHp       = util:ceil(Hp       * Arg),
    NewAtk      = util:ceil(Atk      * Arg),
    NewDef      = util:ceil(Def      * Arg),
    NewPun      = util:ceil(Pun      * Arg),
    NewHit      = util:ceil(Hit      * Arg),
    NewDod      = util:ceil(Dod      * Arg),
    NewCrit     = util:ceil(Crit     * Arg),
    NewCritNum  = util:ceil(CritNum  * Arg),
    NewCritAnit = util:ceil(CritAnit * Arg),
    NewTou      = util:ceil(Tou      * Arg),
    Hero#hero{
        hp         = NewHp
        ,atk       = NewAtk
        ,def       = NewDef
...
%%' gate hero
update_gatehero(Rs) ->
    #role{id = Rid, gatehero = GateHeroList} = Rs,
    Val2 = ?ESC_SINGLE_QUOTES(zip_gatehero(GateHeroList)),
    Sql = list_to_binary([<<"UPDATE `gatehero` SET `hid` = '">>
...
insert_gatehero(Rs) ->
    #role{id = Rid, gatehero = GateHeroList} = Rs,
    Sql = list_to_binary([<<"select count(*) FROM `gatehero` WHERE `role_id` = ">>
            , integer_to_list(Rid)]),
    case db:get_one(Sql) of
        {ok, 0} ->
            Val2 = ?ESC_SINGLE_QUOTES(zip_gatehero(GateHeroList)),
            Sql2 = list_to_binary([<<"INSERT `gatehero` (`role_id`, `hid`) VALUES (">>,
...
init_gatehero(Rs) ->
    #role{id = Rid} = Rs,
    Sql = list_to_binary([<<"select `hid` FROM `gatehero` WHERE `role_id` = ">> ,integer_to_list(Rid)]),
    case db:get_row(Sql) of
        {ok, [Data]} ->
            Data2 = unzip_gatehero(Data),
            Rs1 = Rs#role{gatehero=Data2},
            {ok,Rs1};
        {error, null} ->
            {ok,Rs};
        {error, Reason} ->
...
-define(GATEHERO_ZIP_VERSION, 1).
zip_gatehero([]) -> <<>>;
zip_gatehero(GateList) ->
    zip_gatehero(GateList, []).
zip_gatehero([H | T], Rt) ->
    Rt1 = <<H:32>>,
    zip_gatehero(T, [Rt1 | Rt]);
zip_gatehero([], Rt) ->
    Rt1 = list_to_binary(Rt),
    <<?GATEHERO_ZIP_VERSION:8, Rt1/binary>>.
...
unzip_gatehero(<<H:32,RestBin/binary>>, Ret) ->
    unzip_gatehero(RestBin, [H|Ret]);
unzip_gatehero(<<>>, Ret) ->
    Ret.
%%.
