%% ------------------------------------------------------------------
%% $Id: mod_admin.erl 9223 2014-02-21 02:56:07Z rolong $
%% @doc 后台管理通信及数据统计进程
...
%% 点击后台广播按钮后，会收到一个update_broadcast消息
handle_info(update_broadcast, State) ->
    case get_broadcast() of
        [] -> {noreply, State};
        Broadcast ->
            Bro = get('@send_broadcast'),
            off_send_broadcast(Bro),
            erase('@send_broadcast'),
            get_broadcast(Broadcast),
            {noreply, State}
    end;

handle_info({update_broadcast_send,Id,Ms,Msg,EndTime}, State) ->
    sender:pack_cast(world,27020,[3,0,<<"系统公告">>,Msg,0]),
    case util:unixtime() < EndTime of
        true ->
            Bro = get('@send_broadcast'),
            case lists:keyfind(Id,1,Bro) of
                false -> ok;
                {Id,_TimerRef} ->
                    erlang:send_after(Ms, self(), {update_broadcast_send,Id,Ms,Msg,EndTime})
            end;
        false -> ok
    end,
    {noreply, State};
...
get_notice() ->
    try
        Now = util:unixtime(),
        case db:get_all("SELECT `id`, `msg`, `start_time`, `end_time` FROM `sys_notices` WHERE `end_time` > ~s AND `type` = 1;", [Now]) of
            {ok, Data} ->
...
get_broadcast() ->
    try
        Now = util:unixtime(),
        case db:get_all("SELECT `id`, `time`, `msg`, `start_time`, `end_time` FROM `sys_notices` WHERE `end_time` > ~s AND `type` = 2;", [Now]) of
            {ok, Data} ->
                %% ?INFO("update_notice, count:~w", [length(Data)]),
                [{Id, Time, Msg, StartTime, EndTime} || [Id, Time, Msg, StartTime, EndTime] <- Data];
            {error, null} ->
                [];
            {error, Reason} ->
                ?WARN("Error When get_broadcast: ~w", [Reason]),
                []
        end
        catch T:X ->
            ?ERR("~w:~w", [T, X]),
...
get_broadcast([{Id,Time,Msg,StartTime,EndTime}|R]) ->
    Now = util:unixtime(),
    case Now > StartTime andalso Now < EndTime of
        true ->
            Ms = Time * 60000,
            Bro = case get('@send_broadcast') of
                undefined -> [];
                B -> B
            end,
            Ref = erlang:send_after(Ms, self(), {update_broadcast_send,Id,Ms,Msg,EndTime}),
            put('@send_broadcast', [{Id,Ref}|Bro]),
            get_broadcast(R);
        false ->
            get_broadcast(R)
    end;
...
off_send_broadcast([{_Id,TimerRef}|R]) ->
    erlang:cancel_timer(TimerRef),
    off_send_broadcast(R);
off_send_broadcast(_) -> ok.
