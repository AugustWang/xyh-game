...
battle(Type, AL, BL) ->
    ...
    {Over, Data, SA2HP, SA2SS, SB2SS, Bout} = bouts(Type, SA2, SB2),
    ...
    %%     false -> ok
    %% end,
    SB2SSS = soldiers_heroes(SB2SS),
    print_battle({Over, Data}),
    {Over, InitHp, Data, SA2HP, SA2SS, SB2SSS, Bout}.
...
soldiers_heroes(Soldier) ->
    soldiers_heroes(Soldier,[]).

soldiers_heroes([], Rt) -> Rt;
soldiers_heroes([S | Soldier], Rt) ->
    #s{
        job        = Job
        ,tid       = Tid
        ,pos       = Pos
...
bouts([CurPos | Order], Type, Index, Bout, AL0, BL0, Result) ->
    ...
    if
        IsOver > 0 ->
            ...
            AL0SS = [S || S <- AL0, S#s.hp > 0],
            BL0SS = [S || S <- BL0, S#s.hp > 0],
            {IsOver, lists:reverse(Result), AL0HP, AL0SS, BL0SS, Bout};
        true ->
            ...
            case select_skill(Active, Pos, AL1, BL) of
                {false, AL2} ->
                    ...
                    case select_target_by_pos(Poses, 1, BL, []) of
                        [] -> {AL2, BL, []};
                        AtkedList -> 
                            AtkedList1 = set_damageds(AtkedList, 1),
                            atk_targets(Pos, AL2, BL, AtkedList1, 0)
                    end;
...
do_clean([[Pos, 0, _, _] | Re], AL) ->
    %% ?INFO("DIE:~w", [Pos]),
    case get_s(Pos, AL) of
        false ->
            self() ! {send_code, 1004},
            ?WARN("ERROR ##### : ~w~nRe:~w~nAL:~w", [Pos, Re, AL]),
            AL;
        S ->
...
atk_target1(A, B0, A01_0, SkillId, BuffIds, Rt) ->
    ...
    {A3, B3} = case Hp1 =< 0 of
        true ->
            ...
            %% ?DEBUG("do_after_kill_skill ~nA:~w~nB:~w", [A3_0, B3_0]),
...
print_battle({IsOver, Data}) ->
    case env:get(debug) =:= on orelse get('$mydebug') =:= true of
        true ->
            ...
            F1 = fun([_Bouts, AtkPos, AtkBuff, Targets, SkillId]) ->
...
calc_hit(A, B) ->
    case A#s.hit > B#s.dod of
        true -> 1;
        false ->
           X = min(B#s.dod - A#s.hit * 1.2, 1000), X1 = 1+0.5*(math:pow(10, -6)*X*X/2.25-2*math:pow(10, -3)*X/1.5),
...
calc_def(A, B) ->
    case (B#s.def + max(B#s.tou - A#s.tou_anit, 0)) > A#s.pun of
        true ->
            ...
            1+0.5*(math:pow(10, -6)*X*X/2.25-2*math:pow(10, -3)*X/1.5);
        false ->
...
calc_crit(A, B) ->
    case A#s.crit > B#s.crit_anti + max(B#s.tou - A#s.tou_anit, 0) of
        true ->
            ...
            X1 = (2*0.001*X/1.5-0.000001*X*X/2.25)/2,
            case rate(X1) of
...
calc_crit_num(A) ->
    ...
    1.5-2*(math:pow(10, -6)*Y*Y/2.25-2*math:pow(10, -3)*Y/1.5).
...
calc_atk(A, B, Crit, AtkArg) ->
    ...
    Rt = util:ceil(max((A#s.atk * (1 - min(max(B#s.tou - A#s.tou_anit, 0), 1000) / 2500)) * Def * Crit * Offset * AtkArg, 0)),
