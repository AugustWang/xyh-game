%%----------------------------------------------------
%% $Id: pt_coliseum.erl 12953 2014-05-22 09:32:28Z piaohua $
%% @doc 协议33 - 新版竞技场
            ...
            case db:get_one(Sql) of
                {ok, 0} ->
                    ...
                    case {Rt1, Rt2} of
                            ...
                            case mod_colosseum:init_coliseum(Rs2) of
                                {ok, Rs3} ->
                                    case Rs3#role.coliseum of
                                        [] -> {ok, [129]};
                                        [Id|_] ->
                                        %% [Id,_Lev1,_Exp1,_Pic1,_Num1,_HighR,_PrizeT,_Report1] ->
                                            ...
                                            Rs4 = mod_task:set_task(7,{1,1},Rs3),
                                            {ok, [0], Rs4}
                                    end;
                                {error, Reason} ->
                        ...
                        Else ->
                            ?WARN("Coliseum role(~w), name(~w), Picture(~w) Reg:~w", [Rs#role.id,Name,Picture,Else]),
                            {ok, [127]}
                    end;
                {ok, 1} -> {ok, [1]};
                {error, Reason} ->
                ...
                true ->
                    case lib_role:spend(diamond, Diamond, Rs) of
                        {ok, Rs1} ->
                            ...
                            {ok, [0, Chances + 1, Num1 + Buys], Rs0#role{arena_chance = Chances + 1, coliseum = Coliseum2, save = [vip,coliseum]}};
                        {error, _} ->
                            {ok, [1, 0, 0]}
                    end;
                false -> {ok, [4, 0, 0]}
