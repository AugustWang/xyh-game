...
p(11000, [Type,Rand,Sid,Platform,Account,Password,Signature], Index_) ->
    PlatformL_ = byte_size(Platform),    AccountL_ = byte_size(Account),    PasswordL_ = byte_size(Password),    SignatureL_ = byte_size(Signature),    Data = << Type:8,Rand:32,Sid:16,PlatformL_:16,Platform:PlatformL_/binary,AccountL_:16,Account:AccountL_/binary,PasswordL_:16,Password:PasswordL_/binary,SignatureL_:16,Signature:SignatureL_/binary >>,
    Len = byte_size(Data),
...
p(11014, [Id,Value], Index_) ->
    Data = << Id:8,Value:8 >>,
    Len = byte_size(Data),
    {ok, << Len:16, Index_:8, 11014:16, Data/binary >>};
...
p(13026, [Equid,Jewtid,Ids], Index_) ->
    IdsAL_ = length(Ids),
    IdsR_ = list_to_binary([<<X:32>>||X <- Ids]),
    Data = << Equid:32,Jewtid:32,IdsAL_:16,IdsR_/binary >>,
    Len = byte_size(Data),
    {ok, << Len:16, Index_:8, 13026:16, Data/binary >>};
...
p(13067, [Type], Index_) ->
    Data = << Type:8 >>,
    Len = byte_size(Data),
    {ok, << Len:16, Index_:8, 13067:16, Data/binary >>};
...
p(13068, [Type], Index_) ->
    Data = << Type:8 >>,
    Len = byte_size(Data),
    {ok, << Len:16, Index_:8, 13068:16, Data/binary >>};
...
p(14036, [Id,Mats], Index_) ->
    Data = << Id:32,Mats:32 >>,
    Len = byte_size(Data),
    {ok, << Len:16, Index_:8, 14036:16, Data/binary >>};

p(Cmd, [], Index) -> {ok, <<0:16, Index:8, Cmd:16>>};

p(14037, [Heroid], Index_) ->
    Data = << Heroid:32 >>,
    Len = byte_size(Data),
    {ok, << Len:16, Index_:8, 14037:16, Data/binary >>};
...
p(14039, [Id], Index_) ->
    Data = << Id:32 >>,
    Len = byte_size(Data),
    {ok, << Len:16, Index_:8, 14039:16, Data/binary >>};
...
p(25030, [Id], Index_) ->
    Data = << Id:8 >>,
    Len = byte_size(Data),
    {ok, << Len:16, Index_:8, 25030:16, Data/binary >>};
...
p(25031, [Id], Index_) ->
    Data = << Id:8 >>,
    Len = byte_size(Data),
    {ok, << Len:16, Index_:8, 25031:16, Data/binary >>};
...
p(36004, [Id], Index_) ->
    Data = << Id:32 >>,
    Len = byte_size(Data),
    {ok, << Len:16, Index_:8, 36004:16, Data/binary >>};
...
p(36006, [Id], Index_) ->
    Data = << Id:8 >>,
    Len = byte_size(Data),
    {ok, << Len:16, Index_:8, 36006:16, Data/binary >>};
