%%----------------------------------------------------
%% $Id: pt_role.erl 13040 2014-05-28 07:57:00Z piaohua $
%% @doc 协议11 - 角色相关
...
%%     4、修改密码功能
%%
handle(11000, [Type, Rand, Sid, Platform, Aid, Password, Signature], Rs) ->
                ...
                1 ->
                    ...
                    case db:get_one("select count(*) from role where aid = ~s limit 1", [Aid]) of
                        {ok, 0} ->
                            ...
                            Sql = list_to_binary([
                                    <<"insert into role(gold, diamond, aid, password, name, sid, platform, tollgate_id, tollgate_newid, ctime, kvs) values(">>
                ...
                true ->
                    ...
                    case lib_role:spend(diamond, Diamond, Rs) of
                        {ok, Rs1} ->
                            ...
                            NewPower = Power + data_config:get(tired_max),
...
%% 功能开放进度
handle(11014, [Id,Value], Rs) ->
    #role{fundata = Fundata} = Rs,
    case Id < 256 andalso Value < 256 of
        true ->
            Fundata2 = lists:keystore(Id,1,Fundata, {Id,Value}),
            {ok, [0], Rs#role{fundata = Fundata2,save=[fundata]}};
        false ->
            {ok, [127]}
    end;
...
handle(11104, [], Rs) ->
    ...
    sender:pack_send(Sender, 11015, [Fundata]),
    F = fun(I, {Es, Ps}) ->
            case I#item.tid >= ?MIN_EQU_ID of
    ...
    %% login time
    Rs6 = Rs5#role{loginsign_time = util:unixtime()},
    {ok, [0], Rs6};
