...
class Act_Custom extends Page
{
    ...
    public function process()
    {
        if ($this->input['submit'] == '发送')
        {
            ...
            $custom1 = "{custom_register,".(int)strtotime($this->input['kw']['reg_st']).",".(int)strtotime($this->input['kw']['reg_et'])."}";
            $custom2 = "{custom_tollgate,".(int)$this->input['kw']['lv_st'].",".(int)$this->input['kw']['lv_et']."}";
            $custom3 = "{custom_shop_diamond,".(int)$this->input['kw']['sd_st'].",".(int)$this->input['kw']['sd_et']."}";
            // $arg = array(
            //     array(
            //         $this->input['id'],
            //         $this->input['body'],
            ...
            $rt = $erl->get('lib_admin', 'send_email_custom', '[~s, ~s, ~s, ~u, ~u]', $arg);
            if ($rt[0] == "ok") {
                $log = sprintf("id:%s, body:%s
                    ...
                    ,reg_st:%d,reg_et:%d,lv_st:%d,lv_et:%d
                    ,sd_st:%d,sd_et:%d",
                    $this->input['id'],
                    $this->input['body'],
                    ...
                    //$this->input['heroes_quality'],
                    (int)$this->input['quality'],
                    (int)$this->input['kw']['reg_st'],
                    (int)$this->input['kw']['reg_et'],
                    (int)$this->input['kw']['lv_st'],
                    (int)$this->input['kw']['lv_et'],
                    (int)$this->input['kw']['sd_st'],
                    (int)$this->input['kw']['sd_et']
                );
                Admin::log(28, $log);
