...
class Act_log_custom extends Page
{
    ...
    public function process()
    {
        ...
        $sql1 = "SELECT `ctime`, `body`, `start_time`, `end_time`, `count` FROM `log_custom_stat` WHERE 1 ";
        //if($this->input['month']) {
        ...
        // 生成日期选择下拉列表
        $date_data = $db->getAll("SELECT DISTINCT FROM_UNIXTIME(`ctime`,'%Y%m%d') as day_date FROM `log_custom_stat` WHERE 1 order by `ctime` desc limit 30");
