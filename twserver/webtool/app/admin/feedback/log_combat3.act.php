...
class Act_log_combat3 extends Page
{
    ...
    public function process()
    {
        ...
        $sqlWhere = " and `tollgate_id` >= 20001 and `tollgate_id` <= 30013 ";
        ...
        foreach($data['list'] as $k => $v){
            ...
            foreach($data['gate'][$v['role_id']] as $v2){
                $data['gate1'][$v['role_id']][$v2['tollgate_id']] = $v2['num'];
            }
        }

        for($i=20001;$i<=20088;$i++){
            $data['tollgate'][$i]['t'] = $i;
        }
        for($i=30001;$i<=30013;$i++){
            $data['tollgate'][$i]['t'] = $i;
        }
