...
class Act_log_gold extends Page
{
    ...
    public function process()
    {
        ...
        $sql1 = "SELECT COUNT(DISTINCT `role_id`) as role_num, `type`, SUM(`num`) as num, FROM_UNIXTIME(`ctime`,'%Y%m%d') as time FROM `log_gold` WHERE 1 ";
        if($this->input['month']) {
        ...
        // 生成日期选择下拉列表
        $date_data = $db->getAll("SELECT DISTINCT FROM_UNIXTIME(`ctime`,'%Y%m%d') as day_date FROM `log_gold` WHERE 1 order by `ctime` desc limit 300");
