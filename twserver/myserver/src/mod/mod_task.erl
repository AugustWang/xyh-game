        ...
        {ok, [Bin]} ->
            ...
            case Rs#role.loginsign_time < Time of
                true ->
                    ...
                    Rs2 = already_done(Rs1),
                    {ok, Rs2};
                false ->
                    Rs1 = Rs#role{task = MyTask},
                    Rs2 = already_done(Rs1),
                    {ok, Rs2}
            end;
        {error, null} ->
...
already_done(Rs) ->
    ...
    Task1 = already_done1(Task,Name,GateHero,[]),
    Rs#role{task = Task1}.

already_done1([],_,_,Rt) -> Rt;
already_done1([{Id,Target,[{C1,C2}|L],St}|T],Name,GateHero,Rt) ->
    case Target of
        6 when St == 0 ->
            case lists:member(C1,GateHero) of
                true ->
                    already_done1(T,Name,GateHero,[{Id,Target,[{C1,1}|L],1}|Rt]);
                false ->
                    already_done1(T,Name,GateHero,[{Id,Target,[{C1,C2}|L],St}|Rt])
            end;
        7 when St == 0 ->
            case Name of
                <<>> ->
...
already_done2(6,[{N1,N2}|N],_Name,GateHero) ->
    %% TODO: traversal N
    case lists:member(N1,GateHero) of
        true -> {[{N1,1}|N],1};
        false -> {[{N1,N2}|N],0}
    end;
already_done2(7,[{N1,N2}|N],Name,_GateHero) ->
    case Name of
        <<>> -> {[{N1,N2}|N],0};
        _ -> {[{N1,1}|N],1}
    end;
already_done2(_,N,_,_) -> {N,0}.
...
add_task(Id,Rs) ->
    ...
    case util:get_val(condition,Data) of
        ...
        Con when is_list(Con) ->
            N = [{N1,0} || {N1,_N2} <- Con],
            %% TODO: already done
            {N1,St1} = already_done2(Target,N,Name,GateHero),
            Task1 = lists:keystore(Id,1,Task,{Id,Target,N1,St1}),
