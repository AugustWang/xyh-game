%% ------------------------------------------------------------------
%% $Id: mod_arena.erl 9996 2014-03-13 06:53:10Z piaohua $
%% @doc 竞技场
...
handle_call({get_rank_pos, Id, Lev}, _From, State) ->
    Pos = case get({rank, Lev}) of
        undefined -> 0;
        Data ->
            case lists:keyfind(Id, 2, Data) of
                false -> 0;
                {X, _, _, _, _} -> X
            end
    end,
    ...
    case db:get_row(Sql, [Rs#role.id]) of
        {ok, [Id, Lev, Exp, Pic]} ->
            {ok, Rs#role{
                    ...
                    ,arena_exp = Exp
                    ,arena_picture = Pic
                    ,arena_rank = get_rank_pos(Id, Lev)
                }};
        {error, null} ->
            {ok, Rs};
        {error, Reason} ->
...
get_data(Id) ->
    ...
    case cache:get(Key) of
        undefined ->
            %% util:test1(),
            Sql = <<"SELECT robot, rid, lev, exp, name, s FROM `arena` WHERE id = ~s">>,
...
set_data(Rs, Lev, Exp, Heroes) ->
    ...
    gen_server:cast(cache, {arena_cache, Rs, Lev, Exp, Heroes1}).
...
update_arena_state(Name, Rs) ->
    ...
    case lists:keyfind(Name, 2, ArenaTuple) of
        ...
        {Aid, Aname, Apicture, _} ->
            ...
            F1 = fun(A, B, C, D) ->
                    case B =:= Aname of
                        true -> {Aid, Aname, Apicture, 1};
                        false -> {A, B, C, D}
                    end
            end,
            ArenaList = [F1(A,B,C,D) || [A,B,C,D] <- Rs#role.arena],
...
add_arena_exp(AddExp, Rs) ->
    ...
    Exp = Rs#role.arena_exp,
    Data = data_arena_exp:get(Lev),
    ExpMax = util:get_val(exp_max, Data, 0),
    if
        AddExp == 0 ->
            {Lev, Exp, Rs};
        Data == undefined ->
            ?WARN("undefined arena lev data: ~w", [Lev]),
            {Lev, Exp, Rs};
        ExpMax =< 0 ->
            ?WARN("undefined arena exp_max: ~w", [Data]),
            {Lev, Exp, Rs};
        NewExp == 0 ->
        ...
        NewExp < 0 ->
            case Lev =< 1 of
                ...
                false ->
                    Lev1 = Lev - 1,
                    NextData = data_arena_exp:get(Lev1),
                    NextExpMax = util:get_val(exp_max, NextData, 0),
                    Exp1 = NextExpMax + NewExp,
                    Rs1 = Rs#role{arena_exp = Exp1, arena_lev = Lev1},
                    {Lev1, Exp1, Rs1}
            end;
        NewExp > 0 ->
            case NewExp >= ExpMax of
                true ->
                    %% 升级
                    Lev1 = Lev + 1,
                    case data_arena_exp:get(Lev1) of
                        undefined ->
                            %% 可能已满级
                            {Lev, Exp, Rs};
                        _ ->
...
get_team1(ArenaId) ->
    ...
    case db:get_row(Sql, [ArenaId]) of
        ...
        {error, Reason} ->
            ?WARN("Error When Arena get_team1: ~w, ArenaId: ~w", [Reason, ArenaId]),
            []
    end.
...
get_team2(ArenaId) ->
    ...
    case db:get_one(Sql, [ArenaId]) of
        ...
        {error, Reason} ->
            ?WARN("Error When Arena get_team2: ~w, ArenaId:~w", [Reason, ArenaId]),
            []
    end.
...
%% 获取对手英雄信息
arena_get_rival_heroes(Rid, Rs) ->
    Pid = lib_role:get_role_pid(role_id, Rid),
    case Pid of
        false -> [];
        _ ->
            sender:send_info(Rid, 2048, [Rs#role.pid_sender])
    end.
...
%%     Num = case
%%     Sql = list_to_binary([<<"SELECT `id`, `name`, `exp`, `power` FROM `arena` WHERE `lev` = ">>,
%%             integer_to_list(Lev), <<" ORDER BY `exp` DESC LIMIT 0, ">>, integer_to_list(Num)]),
%%     case db:get_all(Sql) of
%%         {ok, Data} -> fix_rank(Data);
%%         {error, null} ->
%%             [];
