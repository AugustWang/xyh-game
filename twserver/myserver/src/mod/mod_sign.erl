%% ------------------------------------------------------------------
%%
%% $Id: mod_sign.erl 12405 2014-05-06 03:31:06Z piaohua $
%%
%% @doc 登录连续签到
...
send_sign(Rs) ->
    ...
    case LoginSign of
        [] ->
            ...
            Rs1 = Rs#role{loginsign = [Time,1,Days1,Days2,PriceState]},
...
login_sign(Rs) ->
    #role{loginsign = [SignTime,T2,Days1, Days2, PriceState]} = Rs,
    NewTime = util:unixtime(),
    ...
    Rs#role{loginsign = [NewTime,T2,NewDays1,NewDays2,NewPriceState]}.
        ...
        {ok, Bin} ->
            case sign_unzip(Bin) of
                ...
                Sign ->
                    case length(Sign) of
                        3 ->
                            #role{loginsign_time = Time
                                ,loginsign_type = Type} = Rs,
                            {ok, Rs#role{loginsign = [Time,Type|Sign]}};
                        _ ->
...
%% sign_zip / sign_unzip
%% 打包成binary
-define(SIGN_ZIP_VERSION, 2).
sign_zip([T1,T2,X1,X2,[{D1,S1},{D2,S2},{D3,S3},{D4,S4},{D5,S5},{D6,S6},{D7,S7}]]) ->
    <<?SIGN_ZIP_VERSION:8, T1:32,T2:8,X1:8,X2:8,D1:8,S1:8,D2:8,S2:8,D3:8,S3:8,D4:8,S4:8,D5:8,S5:8,D6:8,S6:8,D7:8,S7:8>>.
...
sign_unzip(<<2:8,T1:32,T2:8,X1:8,X2:8,D1:8,S1:8,D2:8,S2:8,D3:8,S3:8,D4:8,S4:8,D5:8,S5:8,D6:8,S6:8,D7:8,S7:8>>) ->
    [T1,T2,X1,X2,[{D1,S1},{D2,S2},{D3,S3},{D4,S4},{D5,S5},{D6,S6},{D7,S7}]];
