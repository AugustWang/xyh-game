        ...
        {3, Type} ->
            ...
            case mod_colosseum:get_data(ArenaId) of
                {Robot, Rid, _Lev, _, Name, S} ->
                    ...
                    Rs7 = case IsWin of
                        1 -> mod_task:set_task(15,{1,1},Rs6);
                        _ -> Rs6
                    end,
                    ?LOG({combat,Rs#role.id,0}),
                    {[Rs7#role.power, 0, IsWin, 0, InitHp, Data2, [], Data3, Data4], Rs7};
                {error, Reason} ->
                ...
                true ->
                    ...
                    case mate_hero(Data,Heroes1) of
                        ...
                        Heroes3 ->
                            ...
                            {Power2, IsWin, Rs9, Data3, Data4, Data5} =
                            case Over =:= 1 of
                                true ->
                                    ...
                                    Rs2 = mod_task:upgrade_task(Heroes1,NewHeroes,Rs),
                                    Rs3 = Rs2#role{
                                        heroes = NewHeroes,
                                    ...
                                    %% Rs4 = case A < B of
                                    %%     true ->
                                    %%         mod_attain:attain_state(8, 1, Rs3);
                                    %%     false ->
                                    %%         Rs3
                                    %% end,
                                    %% Rs5 = mod_attain:attain_state(35, 1, Rs4),
                                    %% 副本开户成就推送
                                    %% Rs6 = mod_attain:attain_state2(60, PassId, Rs5),
                                    %% 战斗胜利请求战斗掉落
                                    #role{produce_pass = ProducePass} = Rs3,
                                    {Rs6, [Data6, Data7]} = mod_item:drop_produce(Rs3, ProducePass),
                                    %% 关卡邮件奖励
                                    ...
                                    %% 成就
                                    Rs8 = case (ReqPassId + 1) > PassId of
                                        true ->
                                            if
                                                ReqPassId == 263 ->
                                    ...
                                    %% Rs777 = combat_attain(Data7,Rs8),
                                    {Power1, 1, Rs8, HeroesInfo, Data6, Data7};
                                false ->
                                    {Power, 0, Rs, [], [], []}
                            end,
                            %% 主线战斗成就推送
                            Rs0 = mod_attain:attain_state(34, 1, Rs9),
                            %% combat 主线战报
                            case IsWin == 1 of
                                true ->
                            ...
                            Rs01 = case Combat > Rs0#role.combat of
                                true -> Rs0#role{combat = Combat};
                                false -> Rs0
                            end,
                            %% nightmare fb
                            {Rs02,Star} = case IsWin == 1 andalso lists:keymember(nightmare,1,Data) of
                                true ->
                                    ...
                                    #role{save = SaveState} = Rs01,
                                    {Rs01#role{fb_nightware = Fbn,save = [fbinfo] ++ SaveState},Star3};
                                false -> {Rs01,0}
                            end,
                            %% task
                            Rs03 = case IsWin of
                                1 ->
                                    Rs02_ = mod_task:set_task(1,{ReqPassId,1},Rs02),
                                    mod_task:set_task(21,{1,1},Rs02_);
                                _ ->
                                    mod_task:set_task(1,{ReqPassId,0},Rs02)
                            end,
                            Rs04 = mod_task:enemy_task(Heroes2,EHEROES,Rs03),
                            ?LOG({combat,Rs#role.id,ReqPassId}),
                            {[Power2, PassId, IsWin, Star, InitHp, Data2, Data3, Data4, Data5], Rs04}
                    end;
                false ->
                ...
                true ->
                    ...
                    case mate_hero(Data,Heroes1) of
                        ...
                        Heroes3 ->
                            ...
                            Rs8 = case IsWin of
                                1 ->
                                    Rs7_ = case lists:keymember(nightmare,1,Data) of
                                        true -> mod_task:set_task(23,{1,1},Rs7);
                                        false -> mod_task:set_task(24,{1,1},Rs7)
                                    end,
                                    mod_task:set_task(1,{ReqPassId,1},Rs7_);
                                _ ->
                        ...
                        {ok, Items1, Notice} ->
                            ...
                            case mate_hero(Data,Heroes1) of
                                ...
                                Heroes3 ->
                                    ...
                                    Rs6 = case IsWin of
                                        1 ->
                                            Rs5_ = mod_task:set_task(1,{ReqPassId,1},Rs5),
                                            mod_task:set_task(22,{1,1},Rs5_);
                                        _ ->
