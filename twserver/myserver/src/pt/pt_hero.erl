%%----------------------------------------------------
%% $Id: pt_hero.erl 13109 2014-06-03 07:32:41Z piaohua $
%% @doc 协议14 - hero
...
%% 增加主线关卡,英雄个数限制
handle(14001, [L], Rs) ->
    #role{tollgate_newid = TollgateNewid} = Rs,
    Count = get_maxcount_by_gateid(TollgateNewid),
    case length(L) =< Count of
        true ->
            case mod_hero:set_pos(Rs, L) of
                {ok, Rs1} ->
                    Rs2 = case length(L) of
                        Count ->
                            mod_attain:attain_state(76,1,Rs1);
                        _ -> Rs1
                    end,
                    {ok, [0], Rs2};
                {error, _} -> {ok, [2]}
                        ...
                        {ok, Rs1} ->
                            case mod_item:del_items(by_tid, Materials, Items) of
                                {ok, Items1, Dels} ->
                                    ...
                                    %%     false -> Rs2
                                    %% end,
                                    Rs3 = case Hero1#hero.quality >= 4 of
                                        true -> mod_attain:attain_state(86, 1, Rs2);
                                        false -> Rs2
                                    end,
                                    Rs4 = mod_attain:attain_state(45, 1, Rs3),
                                    Rs5 = lib_role:spend_ok(CType, 30, Rs, Rs4),
                                    Rs0 = mod_task:set_task(8,{1,1},Rs5),
                                    %% 一切都OK后，从数据库中删除物品
                                    mod_item:del_items_from_db(Rs#role.id, Dels),
                                    mod_item:log_item(del_item,Rs#role.id,12,Materials),
                                    {ok, [0], Rs0};
                                {error, _} ->
                                    {ok, [3]}
                            end;
        ...
        true ->
            ...
            case lib_role:spend(diamond, SearchPrice1, Rs1) of
                {ok, Rs2} ->
                    ...
                    Rs5 = mod_attain:attain_state(32, 1, Rs4),
                    Rs0 = mod_task:set_task(19,{1,1},Rs5),
                        ...
                        {ok, Rs1} ->
                            case length(Rs#role.heroes) < Rs#role.herotab of
                                true ->
                                    ...
                                    %%     false -> Rs5
                                    %% end,
                                    Rs6 = mod_attain:attain_state(73, 1, Rs5),
                                    Rs0 = mod_attain:attain_state(81, 1, Rs6),
                                    ...
                                    ?LOG({hero,Rs#role.id,Tid,Q}),
                                    %% lib_admin:android_server_active3(Rs#role.id,Tid),
                                    extra:set_use(hero,Tid,Rs#role.id),
                                    {ok, [0], Rs0};
                                false ->
                                    ...
                                    %%     false -> Rs2
                                    %% end,
                                    Rs3 = mod_attain:attain_state(81, 1, Rs2),
                                    Rs4 = mod_attain:attain_state(73, 1, Rs3),
                                    case Q >= 6 of
                                        true ->
                                            Name = lib_admin:get_name(Rs),
                                            Vip = lib_admin:get_vip(Rs),
    ...
    case mod_hero:get_hero(HeroId, Heroes) of
        ...
        Hero ->
            ...
            case check_bag_grids(HeroId, Rs) of
                {ok, Rs1} ->
                    AddItemData = data_exp:get(Lev),
                    AddItem = util:get_val(item,AddItemData,[]),
                    case mod_item:add_items(Rs1,AddItem) of
                        {ok, Rs2, PA, EA} ->
                            Heroes1 = lists:keydelete(HeroId, 2, Heroes),
                            mod_hero:db_delete(Rid, HeroId),
                            mod_item:send_notice(Rs2#role.pid_sender, PA, EA),
                            Rs3 = lib_role:add_attr(CType, Price1, Rs2),
                            Rs4 = lib_role:add_attr_ok(CType, 38, Rs, Rs3),
                            lib_role:notice(Rs4),
                            Rs0 = mod_attain:attain_state(72, 1, Rs4),
                            mod_item:log_item(add_item,Rs#role.id,6,AddItem),
...
%%                     ?DEBUG("Rid: ~w, hero fire: ~w ms", [Rs#role.id, DT]),
...
%% 英雄药水
handle(14036, [HeroId, Id], Rs) ->
    %% ?DEBUG("HeroId:~w, Id:~w", [HeroId, Id]),
    #role{heroes = Heroes, items = Items} = Rs,
    LevIds = data_exp:get(ids),
    case mod_hero:get_hero(HeroId, Heroes) of
        false -> {ok, [127,0,0]};
        Hero when Hero#hero.lev < length(LevIds) ->
            case mod_item:get_item(Id,Items) of
                false -> {ok, [128,0,0]};
                Item ->
                    Data = data_prop:get(Item#item.tid),
                    case util:get_val(sort,Data) of
                        5 ->
                            Exp = util:get_val(control1,Data,0),
                            case mod_item:del_item(by_id, Id, 1, Items) of
                                {ok, Items1, Dels} ->
                                    ...
                                    mod_item:del_items_from_db(Rs#role.id, Dels),
                                    mod_hero:hero_notice(Rs#role.pid_sender, Hero1),
                                    Rs1 = mod_task:set_task(10,{1,1},Rs),
                                    {ok, [0,Hero1#hero.lev,Hero1#hero.exp], Rs1#role{heroes = Heroes1,items = Items1}};
                                {error, Reason} ->
                                    ?WARN("potion 14036 hero del_item error: ~w", [Reason]),
                                    {ok, [129, 0, 0]}
                            end;
                        _ -> {ok, [1,0,0]}
                    end
...
%% 英雄培养
handle(14037, [HeroId], Rs) ->
    #role{heroes = Heroes, items = Items} = Rs,
    case mod_hero:get_hero(HeroId, Heroes) of
        false ->
            {ok, [127]};
        Hero ->
            case data_hero_starc:get(Hero#hero.foster + 1) of
                undefined ->
                    {ok, [4]};
                Data ->
                    Num1 = util:get_val(num1, Data),
                    Type = util:get_val(type, Data),
                    Num = util:get_val(num, Data),
                    {CType, RtCode} = case Type of
                        1 -> {gold, 1};
                    ...
                    case lib_role:spend(CType, Num, Rs) of
                        {ok, Rs1} ->
                            case mod_item:del_items(by_tid, [{Hero#hero.tid,Num1}], Items) of
                                {ok, Items1, Dels} ->
                                    Hero1 = mod_hero:upgrade_star(Hero),
                                    Heroes1 = mod_hero:set_hero(Hero1, Heroes),
                                    lib_role:notice(Rs1),
                                    mod_item:send_notice(Rs#role.pid_sender, Dels),
                                    Rs0 = lib_role:spend_ok(CType, 64, Rs, Rs1),
                                    %% 一切都OK后，从数据库中删除物品
                                    mod_item:del_items_from_db(Rs#role.id, Dels),
                                    mod_hero:hero_notice(Rs#role.pid_sender, Hero1),
                                    mod_item:log_item(del_item,Rs#role.id,14,[{Hero#hero.tid,Num1}]),
                                    {ok, [0], Rs0#role{items = Items1, heroes = Heroes1, save = [items,heroes]}};
                                {error, _Reason} ->
                                    %% ?WARN("[14037] hero foster :~w", [Reason]),
                                    {ok, [3]}
                            end;
                        {error, _} ->
                            {ok, [RtCode]}
...
handle(14038, [], Rs) ->
    #role{
        ...
        ,gatehero = GateRoleList
    } = Rs,
    CurTolId = TollgateNewid - 1,
    case data_gate_buy:get(ids) of
        undefined -> {ok, [127, []]};
        HeroList ->
            Data = lists:map(fun(Id) ->
                        TupList = data_gate_buy:get(Id),
                        PointId = util:get_val(pointID,TupList),
                        IsMember = lists:member(Id,GateRoleList),
                        Ret =
                        if  IsMember ->
                                2;
                            CurTolId >= PointId ->
...
%% handle(14038, [], Rs) ->
%%     #role{gatehero = GateRoleList} = Rs,
%%     {ok, GateRoleList};

handle(14039,[Id],Rs) ->
    #role{gatehero = GateHero
        ,tollgate_newid = TollgateNewid
        ,heroes = Heroes
        ,herotab = HeroTab} = Rs,
    case lists:member(Id,GateHero) of
        true -> {ok, [3]};
        false ->
            case data_gate_buy:get(Id) of
                undefined -> {ok, [127]};
                Data ->
                    Tid = util:get_val(heroID,Data),
                    Q = util:get_val(quality,Data),
                    Level = util:get_val(level,Data),
                    Star = util:get_val(star,Data),
                    PointId = util:get_val(pointID,Data),
                    Type = util:get_val(payType,Data),
                    SellPrice = util:get_val(sellPrice,Data),
                    {CType, RtCode} = case Type of
                        1 -> {gold, 1};
                        2 -> {diamond, 6}
                    end,
                    case TollgateNewid - 1 < PointId of
                        true -> {ok, [2]};
                        false ->
                            case length(Heroes) < HeroTab of
                                true ->
                                    case lib_role:spend(CType,SellPrice,Rs) of
                                        {ok,Rs1} ->
                                            %% ?DEBUG("GateHero:~w,Id:~w,Rid:~w",[GateHero,Id,Rs#role.id]),
                                            {ok, Rs2, Hero1} = mod_hero:add_hero(Rs1,Tid,Q),
                                            DataExp = data_exp:get(Level),
                                            MaxExp = util:get_val(exp_sum,DataExp,0),
                                            Hero2 = mod_hero:add_hero_exp(Hero1, MaxExp),
                                            Hero3 = upgrade_star_num(Hero2,Star),
                                            %% Hero3 = Hero2#hero{foster = Star},
                                            %% Hero2 = Hero1#hero{foster = Star,lev=Level,exp_max=MaxExp},
                                            %% self() ! {pt, 2010, [Level]},
                                            Heroes1 = mod_hero:set_hero(Hero3, Heroes),
                                            Rs3 = lib_role:spend_ok(CType, 68, Rs, Rs2),
                                            mod_hero:db_insert(Rs#role.id, Hero3),
                                            mod_hero:hero_notice(Rs#role.pid_sender, Hero3),
                                            lib_role:notice(Rs3),
                                            Rs4 = Rs3#role{gatehero = [Id|GateHero],heroes = Heroes1},
                                            Rs0 = mod_task:set_task(6,{Id,1},Rs4),
                                            case Hero3#hero.quality >= 6 of
                                                true ->
                                                    Name = lib_admin:get_name(Rs),
                                                    Vip = lib_admin:get_vip(Rs),
                                                    H = util:quality2string(Hero3#hero.quality),
                                                    Msg = data_text:get(9),
                                                    Msg1 = io_lib:format(Msg, [Name, H, Tid]),
                                                    lib_system:cast_sys_msg(0, Name, Msg1, Vip);
                                                false -> ok
                                            ...
                                            ?LOG({hero,Rs#role.id,Tid,Q}),
                                            St = Rs0#role.save,
                                            {ok, [0], Rs0#role{save=[heroes,gatehero]++St}};
                                        {error,_} -> {ok,[RtCode]}
                                    end;
                                false -> {ok, [5]}
                            end
...
%%             Type = util:get_val(payType,Data),
%%             SellPrice = util:get_val(sellPrice,Data),
%%             CurPointId =  TollgateNewid - 1,
%%             {CType, _RtCode} = case Type of
%%                 1 -> {gold, 1};
%%                 2 -> {diamond, 2}
%%             end,
%%             {Ret,Val} = lib_role:spend(CType, SellPrice, Rs),
%%             if  CurPointId < PointId ->
%%                     {ok, [2]};
%%                 Ret == error ->
%%                     if  CType == gold ->
%%                             {ok, [1]};
...
%%                     case length(Rs#role.heroes) < Rs#role.herotab of
%%                         true ->
%%                             Rs2 = Rs1#role{gatehero = [Id|GateRoleList]},
%%                             {ok, Rs3, Hero1} = mod_hero:add_hero(Rs2,Tid,Q),
%%                             Hero2 = Hero1#hero{lev = Level,foster = Star},
%%                             mod_hero:hero_notice(Rs#role.pid_sender, Hero2),
%%                             Heroes1 = mod_hero:set_hero(Hero1, Rs3#role.heroes),
%%                             Rs4 = lib_role:spend_ok(CType, 68, Rs, Rs3),
%%                             lib_role:notice(Rs4),
%%                             Rs5 = Rs4#role{heroes = Heroes1,save=[heroes,gatehero]},
%%                             Rs6 = mod_attain:attain_state(73, 1, Rs5),
%%                             Rs7 = mod_attain:attain_state(81, 1, Rs6),
%%                             mod_hero:db_insert(Rs#role.id, Hero1),
%%                             case Hero1#hero.quality >= 6 of
%%                                 true ->
%%                                     Name = lib_admin:get_name(Rs),
...
%%                             ?LOG({hero,Rs#role.id,Tid,Q}),
%%                             self() ! {pt, 2010, [Level]},
%%                             Rs0 = mod_task:set_task(6,{Id,1},Rs7),
%%                             {ok, [0], Rs0};
%%                         false ->
%%                             {ok, [5]}
%%                     end
...
%% 获取英雄最大个数,通过关卡id
get_maxcount_by_gateid(GateId) ->
    Ids = data_hero_total:get(ids),
    Idss = lists:reverse(lists:sort(Ids)),
    get_maxcount_by_gateid(Idss,GateId).
get_maxcount_by_gateid([],_GateId) -> 1;
get_maxcount_by_gateid([Id|Ids],GateId) ->
    case GateId > Id of
        true ->
            Data = data_hero_total:get(Id),
            case util:get_val(maxcount,Data) of
                undefined ->
                    get_maxcount_by_gateid(Ids,GateId);
                Max -> Max
            end;
        false ->
            get_maxcount_by_gateid(Ids,GateId)
...
upgrade_star_num(Hero,Num) ->
    case Num =< 0 of
        true -> Hero;
        false ->
            Hero1 = mod_hero:upgrade_star(Hero),
            upgrade_star_num(Hero1,Num-1)
    end.

%% get_maxcount_by_gateid(0) ->
%%     0;
%% get_maxcount_by_gateid(GateId) ->
%%     IdList = data_hero_total:get(ids),
%%     MaxId = lists:max(IdList),
%%     MinId = lists:min(IdList),
%%     Count =
%%     if  GateId >= MaxId ->
%%             [{maxcount,MaxCount}] = data_hero_total:get(MaxId),
%%             MaxCount;
%%         GateId == MinId ->
%%             [{maxcount,MaxCount}] = data_hero_total:get(MinId),
%%             MaxCount;
%%         GateId < MinId ->
%%             [{maxcount,MaxCount}] = data_hero_total:get(MinId),
%%             MaxCount-1;
%%         true ->
%%             case data_hero_total:get(GateId) of
%%                 undefined ->
%%                     get_maxcount_by_gateid(GateId-1);
%%                 [{maxcount,MaxCount}] ->
%%                     MaxCount
%%             end
%%     end,
