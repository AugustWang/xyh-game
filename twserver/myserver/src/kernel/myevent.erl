%%----------------------------------------------------
%% $Id: myevent.erl 12329 2014-04-29 10:10:08Z piaohua $
%%
%% 事件处理
%%----------------------------------------------------
-module(myevent).
...
del_event(Event) ->
    util:test1(),
    ets:safe_fixtable(?ROLE_EVENTS_TABLE_NAME, true),
    clean_event(ets:first(?ROLE_EVENTS_TABLE_NAME), Event),
    ets:safe_fixtable(?ROLE_EVENTS_TABLE_NAME, false),
    util:test2("clean event").

clean_event('$end_of_table', _) -> true;
clean_event(Key, Event) ->
    case ets:lookup(?ROLE_EVENTS_TABLE_NAME, Key) of
        [{Rid, Events}] ->
            case lists:keymember(Event, 1, Events) of
                true ->
                    Events1 = util:find_all_repeat_key_element(Event, 1, Events),
                    Events2 = Events -- Events1,
                    ets:insert(?ROLE_EVENTS_TABLE_NAME, {Rid, Events2});
                false -> ok
            end;
        _ -> ok
    end,
    clean_event(ets:next(?ROLE_EVENTS_TABLE_NAME, Key), Event).
