...
set(Val) ->
    ...
    gen_server:cast(?MODULE, {set_timeing, Val}).
...
get() ->
    ets:foldl(fun get_timeing_val1/2, [], ?MODULE).

get_timeing_val1(H, Rt) ->
...
handle_cast({set_timeing, Val}, State) ->
    Key = ets:info(?MODULE, size),
    NewKey = get_new_key(Key),
