...
if ($p == 'ios') {
    if(check_version($v, IOS_RELEASE_VERSION) <= 0){
        echo get_server_list(IOS_RELEASE_SERVER_IDS);
    }else{
        echo get_server_list(IOS_CHECK_SERVER_IDS);
    }
}else if ($p == 'windows') {
    echo get_server_list(DEV_SERVER_IDS);
}else if ($p == 'a_360') {
    if(check_version($v, UNION_360_RELEASE_VERSION) <= 0){
