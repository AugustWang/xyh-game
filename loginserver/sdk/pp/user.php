...
class MySocket extends Socket
{
     ...
     * @return status 0xE0000000 参数错误 0xE000000E 用户名已存在 0xE00000DB 数据库错误
     */
    public function create($username, $password,$reg_flags=0)
    {
    
        $cmd = 0xAA000101;

        $pack = array('username' => $username."\0",
                      'password' => $this->_hex2bin($password),
                      'reg_flags'=>$reg_flags,
                );
        
        parent::write($this->_pack($cmd, $pack));
        $body_format = "*";
        $body_format = FALSE;
...
//是二进制token_key数据验证接口
// $rs = MySocket::instance()->login_check(0xAA000022,file_get_contents("php://input","r"));
// echo '<pre>';
// print_r($rs);
...
//字符串token_key数据验证接口
//接收sdk数据。
$rs = MySocket::instance()->postCurl($_GET['token']);
echo $rs;
